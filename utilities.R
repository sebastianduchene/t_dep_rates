...
##############################################
#FUNCTION TO CREATE HIGHLY UNBALANCED TREES. A TREE IS GENERATED WITH A COLLESS PARAMETER THAT IS ABOVE A GIVEN VALUE (330 IS A VALUE OF HIGH UNBALANCE FOR TREES OF 50 TAXA).

sim.bdtreeunbal <- function(b = 0.9, d = 0, stop = "taxa", n = 50, extinct = FALSE, col = 330){

	tree <- sim.bdtree(b = b, d = d, stop = stop, n = n, extinct = extinct)
	
	while(colless(as.treeshape(tree)) < col){
